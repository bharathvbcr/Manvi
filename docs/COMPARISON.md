...
| **Model Context Integrity** | Context accumulated in volatile memory arrays; model-visible prompt drifts from saved logs. | In-memory conversation state. | **Session Log Invariant**: Context projected on demand from the append-only session log; asserted byte-for-byte before every LLM stream. |
...
MANVI is the **unification layer**; DevCouncil is the **components**. The split is by ownership first and by workload second:
- **MANVI — the Go harness (`CGO_ENABLED=0`)**: Drives high-concurrency event loops, SSE streams, multi-provider LLM adapters, policy gates, the session log, and damage-diffed terminal rendering without garbage collection stalls or cgo overhead. This is the part that unifies everything else into something runnable, and the part another application embeds.
- **DevCouncil — the analysis components**: `dcverify` (unified diff parsing, rigor gates, diff↔coverage), `dcstore` (task and lease state on SQLite, ACID), `dcgrep` (ignore-aware repository search on ripgrep's engine) and `devmap` (AST code graph and adjacency). Each is a standalone binary with a JSON-on-stdio contract, owned by DevCouncil, useful to any coding agent that speaks it rather than only this one. DevCouncil is mid-port: as its remaining Python subsystems become Rust/Go, they join this list behind the same contract and the harness does not change shape. `dc-glob` is the exception that proves the rule — a *library* linked into `dcverify`, never a process, paired with `manvi/internal/fnmatch` and pinned by a shared 775-case CPython parity fixture.
- **Resolved, never linked**: MANVI resolves every component the same way — `MANVI_<NAME>_BINARY`, then `PATH`, then a local build — and links none of them; `manvi/dc/*` are only the IPC clients, and `crates/` is a mirrored copy of the component sources for local development, with DevCouncil upstream. Components are optional in the precise sense that matters: without `devmap` the neighbour rule reports `repo_map.unavailable` and `verify.sh` records repo navigation as a gate that **did not run** rather than one that passed; without `dcverify`, `secret_scan`, `stub_detection` and `diff_coverage` report as **degraded** rather than clean. A missing component never becomes a silent pass.
- **Strict Process Boundary**: Harness and components communicate exclusively over child process boundaries (`fork`/`exec`) with line-delimited JSON over stdio. This preserves instantaneous static cross-compilation, avoids shared-memory safety pitfalls, eliminates runtime dependency hell — and is what lets MANVI be embedded in another application without inheriting any component's dependency tree.
