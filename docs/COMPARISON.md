...
- **DevCouncil Component Layer (`devmap`, `dcstore`, `dcverify`, `dcgrep`)**: Executes AST code-graph indexing, CPU-intensive unified diff parsing, regex-free glob matching, ignore-aware repository search on ripgrep's engine, and SQLite ACID state persistence. These are **DevCouncil components**, not MANVI's — DevCouncil is being ported to Rust/Go as a set of building blocks with a JSON-on-stdio contract, usable by any coding agent. MANVI resolves each as a binary from `PATH` (or `MANVI_*_BINARY`) and **links none of them**; `manvi/dc/*` are only the IPC clients, and `crates/` is a mirrored copy for local development. See [`COMPONENTS_AND_HARNESS.md`](COMPONENTS_AND_HARNESS.md).
- **Optional by design**: a missing component degrades visibly rather than silently. Without `devmap` the neighbour rule reports `repo_map.unavailable` and `verify.sh` records repo navigation as a gate that did not run, rather than one that passed.
