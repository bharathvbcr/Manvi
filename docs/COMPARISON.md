...
| **Safety & Policy Sandboxing** | Uncontained shell execution (relies purely on external Docker) or naive regex blocklists; easily bypassed by subshells. | Basic confirm prompts or global permissions; no multi-tiered policy engine. | **5-Tier Policy Ladder** (Root Containment → Immutable Hard Rules → Soft Rules → Grants Ledger → Postures → Operator Escalation). |
...
2. **Headless Execution Plane** (`manvi run`): Scripts and CI pipelines benefit from distinct exit codes: `0` (success), `1` (failure), and `2` (step ceiling reached, preventing partial commit of incomplete work). See [`CLI_AND_CONFIGURATION.md`](CLI_AND_CONFIGURATION.md).
