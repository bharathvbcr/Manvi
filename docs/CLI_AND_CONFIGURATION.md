...
| `agents.max_spawn_depth` | `MANVI_AGENTS_MAX_SPAWN_DEPTH` | `2` | No | `human` | Maximum sub-agent delegation depth. A ceiling in code, not a prompt instruction. |
| `agents.max_fanout` | `MANVI_AGENTS_MAX_FANOUT` | `8` | No | `human` | Maximum concurrent sub-agents per parent. A fan-out request above it is refused, never silently trimmed. |
