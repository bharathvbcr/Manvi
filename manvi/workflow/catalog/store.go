...
func (s Store) Put(p *workflow.Program) (Entry, error) {
	...
	entry := Entry{ID: c.ID, Revision: c.Revision, SHA256: p.Digest(), Description: c.Description, Path: filepath.Join(s.Root, c.ID, name)}
...
func (s Store) listIdentity(root *os.Root, id string) ([]Entry, error) {
	...
	for _, file := range files {
		...
		promoted := current.ID == id && current.Revision == revision && current.SHA256 == p.Digest()
		promotedFound = promotedFound || promoted
		out = append(out, Entry{ID: id, Revision: revision, SHA256: p.Digest(), Description: c.Description, Path: filepath.Join(s.Root, id, name), Promoted: promoted})
	}
	if !promotedFound {
...
func (s Store) Promote(id, revision, sha string) error {
	...
	if _, err := readRegular(dir, "current.json"); err != nil && !errors.Is(err, os.ErrNotExist) {
		return err
	}
	e := Entry{ID: id, Revision: revision, SHA256: sha, Description: c.Description, Path: filepath.Join(s.Root, id, revision+".json"), Promoted: true}
