...
func validateAdvancedEnvelope(kind QueryKind, data json.RawMessage) error {
	...
	case QueryExplore:
		...
		var blast map[string]json.RawMessage
		if err := json.Unmarshal(top["blast_radius"], &blast); err != nil {
			return fmt.Errorf("devmap explore blast_radius is malformed: %w", err)
		}
		if err := validateCountedEnvelope(kind, "blast_radius.layers", blast["layers"]); err != nil {
			return err
		}
	...
	case QueryAffected:
		...
		var blast map[string]json.RawMessage
		if err := json.Unmarshal(top["blast_radius"], &blast); err != nil {
			return fmt.Errorf("devmap affected blast_radius is malformed: %w", err)
		}
		if err := validateCountedEnvelope(kind, "blast_radius.layers", blast["layers"]); err != nil {
			return err
		}
	...
	return validateCountedEnvelope(kind, "result", envelope)
}

func validateCountedEnvelope(kind QueryKind, section string, envelope json.RawMessage) error {
	var counts struct {
