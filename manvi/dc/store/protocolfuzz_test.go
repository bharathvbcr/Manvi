...
// childBound is how long a single child invocation may take. Every request in
// these targets is a local, bounded operation over a small SQLite file or a
// diff already in memory, so this is slack rather than a budget: anything that
// reaches it is a child that will not return, which is the failure being
// looked for and not a slow machine.
const childBound = 60 * time.Second

// runChild runs one binary to completion under childBound and returns its
// stdout and exit status. A child that does not finish is a test failure here
// rather than a returned error, because "it did not return" is the one outcome
// no caller of this boundary can recover from.
...
// assertOneDiagnosedObject is the wire contract both Rust binaries are held to:
