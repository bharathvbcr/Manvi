...
func (s *Server) checkCommand(raw json.RawMessage) (any, *Error) {
	...
	if !(d.Blocked() && d.Severity == policy.Hard) {
		redirect, err := s.commandRedirectDecision(p, scope)
		if err != nil {
			return nil, badRequest("%v", err)
		}
...
//
// With no root it returns nil and the caller records the degradation. A target
// that cannot be resolved to a path is still refused without one, because that
// refusal is a statement about the command's text rather than about a
// repository, and it is the one this surface can make honestly either way.
func (s *Server) commandRedirectDecision(p CommandCheckParams, scope *dc.Task) (*policy.Decision, error) {
	targets, opaque, err := policy.RedirectTargets(p.Command)
	if err != nil {
		return nil, fmt.Errorf("policy.check.command: %v", err)
	}
	if opaque {
