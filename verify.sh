...
if (( FUZZ )); then
  ...
  # test suite, the linters and nilaway have already saturated the machine.
  # Unbounded, the engine reported `context deadline exceeded` — its own
  # coordination timing out, with no failing input written — and a gate that
  # fails for reasons unrelated to the code is a gate people learn to skip.
  ...
  while IFS= read -r decl; do
    ...
    if ! out="$( (cd manvi && go test -run '^$' -fuzz "^${fn}\$" -fuzztime "$FUZZTIME" \
  ...
  printf '    covered: all %s declared targets executed on %s workers, %s inputs total at %s each\n' \
    "$fuzz_ran" "$fuzz_workers" "$fuzz_execs" "$FUZZTIME"
fi
